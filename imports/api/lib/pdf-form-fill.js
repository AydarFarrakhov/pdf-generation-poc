const hummus = Npm.require('hummus'),
...
function writeAppearanceXObjectForText(handles, formId, fieldsDictionary, text, inheritedProperties) {
  ...
  const x = rect[ 2 ].value - rect[ 0 ].value;
  const y = rect[ 3 ].value - rect[ 1 ].value;
...
function updateTextValue(handles, fieldDictionary, value, isRich, inheritedProperties) {
  ...
  const appearanceInField = fieldDictionary.exists('Subtype') && (fieldDictionary.queryObject('Subtype').toString() === 'Widget') || !fieldDictionary.exists('Kids');
